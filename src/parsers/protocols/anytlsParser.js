...
export function parseAnytls(url) {
    ...
    const password = decodeComponent(parsedUrl.username || getFirstParam(parsedUrl.searchParams, ['password', 'auth']) || '');
    ...
    const serverName = getFirstParam(params, ['sni', 'servername', 'peer', 'host']);
    if (serverName) {
    ...
    const fingerprint = getFirstParam(params, ['client-fingerprint', 'fingerprint', 'fp']);
    if (fingerprint) {
        tls.utls = {
            enabled: true,
            fingerprint
